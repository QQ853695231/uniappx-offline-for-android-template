...
open class GenUniModulesCoolUiComponentsClImageClImage : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesCoolUiComponentsClImageClImage) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesCoolUiComponentsClImageClImage
            val _cache = __ins.renderCache
            val props = __props
            fun emit(event: String, vararg do_not_transform_spread: Any?) {
                __ins.emit(event, *do_not_transform_spread)
            }
            val pt = computed(fun(): PassThrough7 {
                return parsePt<PassThrough7>(props.pt)
            }
            )
            ...
            return fun(): Any? {
                ...
                return _cE("view", _uM("class" to _nC(_uA(
                    ...
                    )), "src" to _ctx.src, "mode" to _ctx.mode, "lazy-load" to _ctx.lazyLoad, "webp" to _ctx.webp, "show-menu-by-longpress" to _ctx.showMenuByLongpress, "onLoad" to onLoad, "onError" to onError, "onClick" to onTap), null, 42, _uA(
                        "src",
                        "mode",
                        "lazy-load",
                        "webp",

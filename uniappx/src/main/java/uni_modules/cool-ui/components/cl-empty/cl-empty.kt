...
open class GenUniModulesCoolUiComponentsClEmptyClEmpty : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesCoolUiComponentsClEmptyClEmpty) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesCoolUiComponentsClEmptyClEmpty
            val _cache = __ins.renderCache
            val props = __props
            val pt = computed(fun(): PassThrough23 {
                return parsePt<PassThrough23>(props.pt)
            }
            )
            return fun(): Any? {
                        ...
                        _cV(_component_cl_text, _uM("key" to 1, "pt" to object : UTSJSONObject() {
                            var className = unref(parseClass)(_uA(
                                ...
                                object : UTSJSONObject() {
                                    var `-important-text-surface-100` = unref(isDark)
                                }
                            ))
        ...
        val styles0: Map<String, Map<String, Map<String, Any>>>
            get() {
                return _uM("cl-empty" to _pS(_uM("display" to "flex", "height" to "100%", "width" to "100%", "flexDirection" to "column", "alignItems" to "center", "justifyContent" to "center", "pointerEvents" to "none")), "cl-empty--fixed" to _pS(_uM("position" to "fixed", "top" to 0, "left" to 0, "zIndex" to -1)), "cl-empty__icon" to _pS(_uM("marginBottom" to "20rpx")), "cl-empty__text" to _pS(_uM("fontSize" to "24.5rpx", "lineHeight" to "35rpx", "color" to "rgba(161,161,170,1)")))
            }

...
open class GenUniModulesCoolUiComponentsClSwitchClSwitch : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesCoolUiComponentsClSwitchClSwitch) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesCoolUiComponentsClSwitchClSwitch
            val _cache = __ins.renderCache
            val props = __props
            fun emit(event: String, vararg do_not_transform_spread: Any?) {
                __ins.emit(event, *do_not_transform_spread)
            }
            val pt = computed(fun(): PassThrough16 {
                return parsePt<PassThrough16>(props.pt)
            }
            )
            ...
            val rect = computed<Rect>(fun(): Rect {
                ...
                return Rect(height = height + "rpx", width = width + "rpx", size = size + "rpx", left = left + "rpx", translateX = translateX + "rpx")
            }
            )
            fun gen_onTap_fn() {
        ...
        val styles0: Map<String, Map<String, Map<String, Any>>>
            get() {
                return _uM("cl-switch" to _uM("" to _uM("display" to "flex", "flexDirection" to "row", "alignItems" to "center", "transitionDuration" to "200ms"), ".cl-switch--disabled" to _uM("opacity" to 0.5)), "cl-switch__track" to _uM("" to _uM("position" to "relative", "display" to "flex", "flexDirection" to "row", "alignItems" to "center", "borderTopLeftRadius" to 9999, "borderTopRightRadius" to 9999, "borderBottomRightRadius" to 9999, "borderBottomLeftRadius" to 9999, "transitionDuration" to "200ms", "backgroundColor" to "rgba(228,228,231,1)"), ".is-dark" to _uM("backgroundColor" to "rgba(113,113,122,1)"), ".is-checked" to _uM("backgroundColor" to "rgba(20,184,166,1)")), "cl-switch__thumb" to _pS(_uM("position" to "absolute", "display" to "flex", "alignItems" to "center", "justifyContent" to "center", "borderTopLeftRadius" to 9999, "borderTopRightRadius" to 9999, "borderBottomRightRadius" to 9999, "borderBottomLeftRadius" to 9999, "backgroundColor" to "rgba(255,255,255,1)", "transitionDuration" to "300ms")), "@TRANSITION" to _uM("cl-switch" to _uM("duration" to "200ms"), "cl-switch__track" to _uM("duration" to "200ms"), "cl-switch__thumb" to _uM("duration" to "300ms")))
            }

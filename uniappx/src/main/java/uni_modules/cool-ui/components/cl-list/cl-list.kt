...
open class GenUniModulesCoolUiComponentsClListClList : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesCoolUiComponentsClListClList) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesCoolUiComponentsClListClList
            val _cache = __ins.renderCache
            val props = __props
            val pt = computed(fun(): PassThrough20 {
                return parsePt<PassThrough20>(props.pt)
            }
            )
            return fun(): Any? {
                ...
                return _cE("view", _uM("class" to _nC(_uA(
                    "cl-list",
                    _uA(
                        _uM("dark-colon--important-border-surface-700" to unref(isDark)),
                        ...
                        _cE(Fragment, null, RenderHelpers.renderList(_ctx.list, fun(item, index, __index, _cached): Any {
                            return _cE("view", _uM("key" to index), _uA(
                                _cV(_component_cl_list_item, _uM("icon" to item.icon, "label" to item.label, "arrow" to item.arrow, "hoverable" to item.hoverable, "pt" to object : UTSJSONObject() {
                                    var className = "bg-white  " + pt.value.item?.className
                                    ...
                                    return _uA(
                                        renderSlot(_ctx.`$slots`, "item", GenUniModulesCoolUiComponentsClListClListSlotDataItem(item = item), fun(): UTSArray<Any> {
                                            return _uA(

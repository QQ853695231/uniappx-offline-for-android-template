...
open class GenUniModulesCoolUiComponentsClToastClToast : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesCoolUiComponentsClToastClToast, _arg1: SetupContext) -> Any? = fun(__props, ref1): Any? {
            var __expose = ref1.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesCoolUiComponentsClToastClToast
            ...
            fun gen_open_fn(options: ClToastOptions) {
                val item = reactive<ToastItem>(ToastItem(id = id++, visible = true, isOpen = false, icon = options.icon, image = options.image, duration = options.duration ?: 2000, position = options.position ?: "center", message = options.message))
                            ...
                            _cE("view", _uM("class" to "cl-toast-wrapper"), _uA(
                                _cE("view", _uM("class" to _nC(_uA(
                                    ...
                                    _cE("view", _uM("class" to "flex flex-row justify-center"), _uA(
                                        if (item.icon != null) {
                                            _cV(_component_cl_icon, _uM("key" to 0, "name" to item.icon, "size" to 56, "pt" to object : UTSJSONObject() {
                                                var className = "mb-1 -important-text-white"
                                            }), null, 8, _uA(
                                                "name"
                                            ))
                                        } else {

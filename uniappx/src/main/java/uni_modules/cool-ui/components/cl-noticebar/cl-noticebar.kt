...
open class GenUniModulesCoolUiComponentsClNoticebarClNoticebar : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesCoolUiComponentsClNoticebarClNoticebar) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesCoolUiComponentsClNoticebarClNoticebar
            val _cache = __ins.renderCache
            val props = __props
            val proxy = getCurrentInstance()!!.proxy
            val windowWidth = uni_getWindowInfo().windowWidth
            val pt = computed(fun(): PassThrough50 {
                return parsePt<PassThrough50>(props.pt)
            }
            )
                    ...
                    _cE("view", _uM("class" to _nC(_uA(
                        ...
                        _cE(Fragment, null, RenderHelpers.renderList(list.value, fun(item, index, __index, _cached): Any {
                            return _cE("view", _uM("key" to index, "class" to "cl-noticebar__item", "style" to _nS(_uM("height" to unref(parseRpx)(_ctx.height!!)))), _uA(
                                renderSlot(_ctx.`$slots`, "text", GenUniModulesCoolUiComponentsClNoticebarClNoticebarSlotDataText(item = item), fun(): UTSArray<Any> {
                                    return _uA(
        ...
        val styles0: Map<String, Map<String, Map<String, Any>>>
            get() {
                return _uM("cl-noticebar__scroller" to _uM("" to _uM("display" to "flex", "transitionProperty" to "transform", "transitionTimingFunction" to "linear"), ".is-horizontal" to _uM("flexDirection" to "row", "overflow" to "visible"), ".is-vertical" to _uM("flexDirection" to "column", "transitionDuration" to "0.5s")), "cl-noticebar__item" to _pS(_uM("display" to "flex", "flexDirection" to "row", "alignItems" to "center")), "cl-noticebar__text" to _pS(_uM("whiteSpace" to "nowrap")), "@TRANSITION" to _uM("cl-noticebar__scroller" to _uM("property" to "transform", "timingFunction" to "linear", "duration" to "0.5s")))
            }

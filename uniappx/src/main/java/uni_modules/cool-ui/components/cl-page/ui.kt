...
open class GenUniModulesCoolUiComponentsClPageUi : VueComponent {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenUniModulesCoolUiComponentsClPageUi) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesCoolUiComponentsClPageUi
            ...
            fun gen_showTips_fn(message: String, callback: (action: ClConfirmAction) -> Unit) {
                if (tipsRef.value != null) {
                    tipsRef.value!!.open(ClConfirmOptions(title = t("提示"), message = message, callback = callback, showCancel = false))
                }
            }
            ...
            return fun(): Any? {
                val _component_cl_confirm = resolveEasyComponent("cl-confirm", GenUniModulesCoolUiComponentsClConfirmClConfirmClass)
                val _component_cl_toast = resolveEasyComponent("cl-toast", GenUniModulesCoolUiComponentsClToastClToastClass)

...
open class GenUniModulesCoolUiComponentsClConfirmClConfirm : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesCoolUiComponentsClConfirmClConfirm, _arg1: SetupContext) -> Any? = fun(__props, ref1): Any? {
            var __expose = ref1.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesCoolUiComponentsClConfirmClConfirm
            val _cache = __ins.renderCache
            val visible = ref(false)
            val closed = ref(true)
            val config = reactive<ClConfirmOptions>(ClConfirmOptions(title = "", message = ""))

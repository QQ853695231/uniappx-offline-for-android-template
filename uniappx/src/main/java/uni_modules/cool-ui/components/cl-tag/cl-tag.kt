...
open class GenUniModulesCoolUiComponentsClTagClTag : VueComponent {
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesCoolUiComponentsClTagClTag) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesCoolUiComponentsClTagClTag
            val _cache = __ins.renderCache
            val props = __props
            fun emits(event: String, vararg do_not_transform_spread: Any?) {
                __ins.emit(event, *do_not_transform_spread)
            }
            val pt = computed(fun(): PassThrough21 {
                return parsePt<PassThrough21>(props.pt)
            }
            )
                    ...
                    _cE("view", _uM("key" to 0, "class" to _nC(_uA(
                        ...
                        _cV(_component_cl_text, _uM("color" to color.value, "pt" to object : UTSJSONObject() {
                            var className = unref(parseClass)(_uA(
                                "cl-tag__text -important-text-sm",
